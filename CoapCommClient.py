...
class CoapCommClient:
    ...
    def runTests(self):
        ...
        print("Executing all CoAP t0ests")
                
        '''
        NOTE: each call must be made with a new client!
        '''
        self.handleDiscoverTest()
        self.handleGetTest("Trigger")
        self.path = "Trigger"
        for x in range(1, 6):
   
            payload = self.createJsonPayload()
            self.handlePostTest(self.path, payload)
        
        
    def motionData(self):
        print("Motion sensor data")
        sensorValue = random.randint(1, 2)
        return sensorValue

    def createJsonPayload(self):
        time = str(datetime.now())
        sensorValue = self.motionData()
        newJson = {"Group Name":"Home Automation","Name":"SHoLS","Location":"Home","TimeStamp":time,"SensorData":sensorValue}
                    
        
        jsonPayload = json.dumps(newJson)
